...
class BasePromptStrategy(ABC):
    ...
    def validate_parameters(
        ...
        if detail_level not in valid_levels:
            raise ValueError(f"Invalid detail level: {detail_level}. Must be one of {valid_levels}")
        
        if not self.supports_target_type(target_type):
            raise ValueError(f"Strategy {self.get_strategy_name()} does not support target type: {target_type}")
