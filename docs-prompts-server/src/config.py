...
class ConfigurationManager:
    ...
    def __init__(
        self, config_path: Optional[Path] = None, project_root: Optional[Path] = None
    ):
    ...
    def _validate_and_normalize_paths(self, config: Dict[str, Any]):
        ...
        enforce_relative = path_resolution_config.get(
            "enforce_project_root_relative", True
        )
        normalize_absolute = path_resolution_config.get(
            "normalize_absolute_paths", True
        )
        ...
        if not enforce_relative:
            logger.warning(
                "Path resolution enforcement is disabled - paths may not be project-root relative"
            )
            return
        ...
        for path_str in documentation_paths:
            ...
            if path_obj.is_absolute():
                if not allow_absolute:
                    if normalize_absolute:
                        ...
                        try:
                            ...
                            logger.info(
                                f"Normalized absolute path '{path_str}' to relative path '{relative_path}'"
                            )
                            normalized_paths.append(str(relative_path))
                        except ValueError:
                            # Path is not within project root
                            logger.error(
                                f"Absolute path '{path_str}' is not within project root '{self.project_root}' and absolute paths are not allowed"
                            )
                            raise ValueError(
                                f"Documentation path '{path_str}' must be relative to project root or within project root directory"
                            )
                    else:
                        logger.error(
                            f"Absolute documentation paths are not allowed: '{path_str}'"
                        )
                        raise ValueError(
                            f"Documentation path '{path_str}' must be relative to project root"
                        )
                else:
                ...
                if ".." in path_str:
                    logger.warning(
                        f"Path '{path_str}' contains '..' which may navigate outside project root"
                    )
                normalized_paths.append(path_str)
        ...
        logger.info(
            f"Validated {len(unique_paths)} documentation paths as project-root relative"
        )
